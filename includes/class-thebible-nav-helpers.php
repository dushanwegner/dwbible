 ...
 *
 * Extracted from TheBible_Plugin::inject_nav_helpers() to keep the
...
class TheBible_Nav_Helpers {
    ...
    private static function compute_nav_urls($nav, $slug, $bible_index) {
        ...
        $slug_for_urls = get_query_var(TheBible_Plugin::QV_SLUG);
    ...
    private static function build_sticky_bar($book_label, $nav, $nav_ctx, $highlight_ids, $chapter_scroll_id, $bible_index) {
        ...
        if (is_array($nav)) {
            ...
            if (is_string($nav_book) && $nav_book !== '') {
                $book_slug_for_data = TheBible_Plugin::slugify($nav_book);
            }
            if ($nav_ch > 0) {
                $initial_ch = $nav_ch;
            }
        ...
        if ($book_slug_for_data === '') {
            $book_slug_for_data = TheBible_Plugin::slugify($book_label);
        }
        $q_ch = absint(get_query_var(TheBible_Plugin::QV_CHAPTER));
        ...
        return '<div class="thebible-sticky" data-slug="' . $book_slug_js . '"' . $data_attrs . $ch_picker_attrs . '>'
            ...
            . '<a href="' . $nav_ctx['prev_href'] . '" class="thebible-ctl thebible-ctl-prev" data-prev aria-label="Previous chapter">&#8592;</a>'
    ...
    /**
     * Call private static methods on TheBible_Plugin via reflection.
     *
     ...
     *
     * @param string $method Method name on TheBible_Plugin.
    ...
    private static function call_plugin($method, ...$args) {
        ...
        if (!isset($methods[$method])) {
            $ref = new ReflectionMethod('TheBible_Plugin', $method);
